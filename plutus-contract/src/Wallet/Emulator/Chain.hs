...
instance Pretty ChainEvent where
  pretty = \case
    ...
    TmpLogEvent sl s -> "TmpLogEvent" <+> "Slot:" <+> pretty sl <+> "Pool:" <+> pretty s
...
validateBlock params slot@(Slot s) idx txns =
  let -- Select those transactions that can be validated in the
      ...
      (processed, Index.ValidationCtx idx' _) =
        flip S.runState (Index.ValidationCtx idx params) $
          for eligibleTxns $ \tx -> do
...
getCollateral idx tx =
  fromRight (getCardanoTxFee tx) $
